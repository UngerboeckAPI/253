...
namespace Examples.Operations
{
  public class Relationships : Base
  {
    ...
    /// <summary>
    /// You can add two accounts that are already related using this bypass option (provided the new relationship type is unique)
    /// </summary>
    public RelationshipsModel AddWithoutAccountsAlreadyRelatedWarning(string orgCode, string masterAccountCode, string subordinateAccountCode, string relationshipType1, string relationshipType2)
    {
      var myRelationship = new RelationshipsModel
      {
        MasterOrganizationCode = orgCode,
        ...
        SubordinateOrganizationCode = orgCode,
        EventSalesDesignation = "A"
      };

      var options = new Ungerboeck.Api.Models.Options.Subjects.Relationships() { AllowMultipleRelationshipsOfDifferentTypes = true };
      apiClient.Endpoints.Relationships.Add(myRelationship, options);

      //Add it again with a second unique type and it will work with the bypass option
      myRelationship.RelationshipType = relationshipType2;
      return apiClient.Endpoints.Relationships.Add(myRelationship, options);
