...
namespace Examples.Operations
{
  public class ServiceOrderItems : Base
  {
    ...
    /// <summary>
    /// Examples selecting a User Field from an order item
    /// </summary>
    /// <param name="orgCode">Organization Code where the search will take place.  User fields are organization-based.</param>
    /// <returns></returns>
    public SearchResponse<ServiceOrderItemsModel> SelectUserFields(string orgCode)
    {
      //This is searching for Order Items and selecting on user fields where format for the field is Issue Type code = SU and User Number 01 (AMT_01). 
      return apiClient.Endpoints.ServiceOrderItems.Search(orgCode, "OrderNumber eq 153448 and OrderLineNumber eq 1 $select=Event,Description,SU|UserNumber01");
    }
    ...
    /// <param name="orgCode">Organization Code where the search will take place.  User fields are organization-based.</param>
    /// <returns></returns>
    public SearchResponse<ServiceOrderItemsModel> SelectOrderItemsByUserFields(string orgCode)
    {
      //This is searching for Order items user fields of Issue Class = C, Issue Type code = SU, organization code = 10, and User Number 01 (AMT_01).  It will return order items where the value is 1.
      return apiClient.Endpoints.ServiceOrderItems.Search(orgCode, "C|SU|10|UserNumber01 eq 1");
    }
  }
}
